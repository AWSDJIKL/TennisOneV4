    ...
    try:
        while True:
            ...
            if data.get("type") == "start":
                ...
                elif data.get("source") == "camera":
                    # 使用 gxipy 调用工业相机
                    video_path = "gx_camera"

                if not video_path and data.get("source") != "camera":
                    await websocket.send_json(
                        {"type": "error", "message": f"Video not found"}
                    )
                    continue

                if data.get("source") != "camera" and not Path(video_path).exists():
                    await websocket.send_json(
                ...
                try:
                    async for result in pipeline.process_video(
                        ...
                        target_fps=20,
                        skip_frames=1,
                        buffer_seconds=5.0,  # 超参数：提前在后台缓冲处理5秒，再往前端发送，确保不抖动
                    ):
                        if "error" in result:
                            await websocket.send_json(
